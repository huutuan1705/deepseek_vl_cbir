...
class CIRPlus(nn.Module):
    def __init__(self, args, tau=0.01,
        ...
        self.device = device
        self.plus = plus
        self.siglip = Siglip_Retrieval(args)
    ...
    def load_ckpt(self, model_path, is_origin=False):
        ...
        if is_origin:
            self.siglip.load_state_dict(saved_state_dict["SigLIP_Retrieval"], strict=False)
        else:
    ...
    def extract_bank_features(self, cirDataset: CIRDataset, device, bank_path, reload_bank=False):
        ...
        if not os.path.exists(bank_path) or reload_bank:
            ...
            for reference_image, caption, target_image, index, \
                ...
                with torch.no_grad():
                    batch_refer_features = self.siglip.img_embed(reference_image)
                    batch_target_features_p = self.siglip.img_embed(target_image, return_pool_and_normalized=True)[
                        -1].detach().cpu()
                    batch_refer_features_p = F.normalize(
                        self.siglip.vision_proj(batch_refer_features[:, 0, :])).detach().cpu()
    ...
    def extract_refer_bank_features(self, cirDataset: CIRDataset, device, bank_path, reload_bank=False):
        ...
        if not os.path.exists(bank_path) or reload_bank:
            ...
            for reference_image, caption, target_image, index, \
                ...
                target_image = target_image.to(device, non_blocking=True)
                with torch.no_grad():  
                    batch_refer_features = self.siglip.img_embed(reference_image).detach().cpu()
                    batch_target_features = self.siglip.img_embed(target_image).detach().cpu()
    ...
    def bank_large_step(self, loss, text, indexs, target_indexs,
        ...
        query_feats = self.siglip.img_txt_fusion(reference_image_feats, None, text)
